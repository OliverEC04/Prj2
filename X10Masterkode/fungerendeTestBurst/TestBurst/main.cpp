...
{									 // initiering
	...
	{
		if (checkReset())
		{								   // tjek om der er trykket p� resetknappen
		...
		if (minTrigger)
		{ // tjek om der er g�et et minut
			minTrigger = false;
			incrementTime(currentTime);
			alarmStatus = 0; // reset alarmstatus s� alarmer kan k�res igen.
		}
		// tjek alarmer
		if (idle && (alarmStatus << 4) != 0b11110000)
		{										   // hvis alle on-alarmer er k�rt dette minut beh�ves der ikke tjekkes for flere alarmer.
