...
volatile bool burstEnabled = false; // n�r true sendes der burst

volatile bool intTrigger = false; // s�ttes true n�r der er modtaget any-edge interrupt fra firkantsignal

volatile bool idle = true; // angiver om vi er igang med at burste

volatile uint8_t alarmStatus = 0; // hver bit representerer om en kommando er blevet k�rt i dette minut.
...
int buzzerDelay = 20; // antal sekunder buzzeren skal k�re
...
void setupX10()
{
	...
	// reset-knap
	DDRA = 0; // SW-knapper s�ttes som input, er active low
}

void sendBurst() // sender burst i 1 ms
{
	burstEnabled = true;
	_delay_us(340); // pr�vet mig frem til, har ca 1 ms varighed
	burstEnabled = false;
}
...
void runCommand(int *command) // sender kommando
{
	...
	intTrigger = false; // vent p� n�ste edge fra firkantsignal
	while (!intTrigger)
	{
	}

	for (int i = 0; i < 12; i++) // genneml�b givne kommando
	{
		if (command[i] == 1) // hvis 1: send et burst, ellers ikke
		{
			sendBurst();
		}
		intTrigger = false; // vent p� n�ste edge
		while (!intTrigger)
		{
		}
	}

	idle = true; // f�rdig med at k�re kommando
}
...
void chooseCommand(int res)
{
	...
	case 1:
		...
		{								  // tjek om valgte kommando allerede er k�rt dette minut
			runCommand(slave1_buzzer_on); // k�r valgt kommando (med resultat fra checkTime(currentTime))
			alarmStatus |= 0b00000001;	  // s�t status bit'et for kommandoen
	...
	case 2:
		if (!(alarmStatus & 0b00000010))
		{
			...
			coffeeTime = currentTime + coffeeDelay; // s�t tiden den skal slukkes
		}
		break;
	case 3:
